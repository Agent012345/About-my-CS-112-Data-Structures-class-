...
public class CatIsland {
    ...
    public static void solveIsland4(Island islandFour) throws CatInWaterException {
        Tile[][] tiles = islandFour.getTiles(); //for the entire Island, NOT just one tile
    ...
    public static void solveIsland5Recursive(Cat cat, Tile[][] tiles, Tile prev) throws CatInWaterException {
        ...
        if (row == 0 && col == 8){
            ...
            if (row >= 0 && row < tiles.length && col+1 >=0 && col+1 < tiles[0].length && tiles[row][col+1].type.equals(Tile.LAND) && tiles[row][col+1] != prev){
                cat.moveRight();
                solveIsland5Recursive(cat, tiles, tiles[row][col]);
            }
            if (row+1 >= 0 && row+1 < tiles.length && col >=0 && col < tiles[0].length && tiles[row+1][col].type.equals(Tile.LAND) && tiles[row+1][col] != prev){
                cat.moveDown();
                solveIsland5Recursive(cat, tiles, tiles[row][col]);
            }
            if (row >= 0 && row < tiles.length && col-1 >=0 && col-1 < tiles[0].length && tiles[row][col-1].type.equals(Tile.LAND) && tiles[row][col-1] != prev){
